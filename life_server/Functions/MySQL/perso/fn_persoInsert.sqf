...
_query = switch(_side) do
{
	case west:
	{
		format["updatePersoWest:%2:%1",_insert, _uid];
	};
	case independent:
	{
		format["updatePersoIndependent:%2:%1",_insert, _uid];
	};
	case civilian:
	{
		format["updatePersoCivilian:%2:%1",_insert, _uid];
	};
	//case east:
	//{
	//	format["updatePersoEast:%2:%1",_insert, _uid];
	//};
};
